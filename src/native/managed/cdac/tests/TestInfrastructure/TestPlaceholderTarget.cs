...
public class TestPlaceholderTarget : Target
{
    ...
    /// </summary>
    public TestContractRegistry SetupContractRegistry(Action<ContractRegistry>? registrations = null)
    {
    ...
    public static unsafe T ReadFromSpan<T>(ReadOnlySpan<byte> bytes, bool isLittleEndian) where T : unmanaged, IBinaryInteger<T>, IMinMaxValue<T>
    {
    ...
    public static unsafe void WriteToSpan<T>(T value, bool isLittleEndian, Span<byte> dest) where T : unmanaged, IBinaryInteger<T>, IMinMaxValue<T>
    {
