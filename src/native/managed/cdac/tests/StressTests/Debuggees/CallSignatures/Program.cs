...
internal static unsafe class Program
{
    ...
    // Exercises the ClassifyEightBytes port. On non-Unix-x64 targets these
    // methods still run but the classifier self-gates and returns "not in
    // registers" -- the runtime and cDAC agree via other paths and no
    // divergence surfaces. On Linux/macOS x64 the runtime uses eightbyte
    // classification to decide FP-reg vs GP-reg vs stack placement, so
    // ArgIterator produces different GCRefMap tokens depending on which
    // eightbytes end up in which register class.
