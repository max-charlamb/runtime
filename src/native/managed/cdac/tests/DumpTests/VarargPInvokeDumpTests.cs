...
public class VarargPInvokeDumpTests : DumpTestBase
{
    ...
    [SkipOnVersion("net10.0", "InlinedCallFrame.Datum was added after net10.0")]
    [SkipOnOS(IncludeOnly = "windows", Reason = "VarargPInvoke debuggee uses msvcrt.dll (Windows only)")]
    public void VarargPInvoke_HasPInvokeFrame(TestConfiguration config)
    {
        InitializeDumpTest(config);
        IStackWalk stackWalk = Target.Contracts.StackWalk;
        ...
        Assert.True(foundPInvoke, "Expected to find a PInvoke MethodDesc on the crashing thread stack");
    }
    ...
    public unsafe void VarargPInvoke_GetMethodTableDataForILStubFrame(TestConfiguration config)
    {
        ...
        foreach (IStackDataFrameHandle frame in frames)
        {
            ...
            // The ILStub's MethodDesc chunk MethodTable pointer is actually an EEClass
            // pointer, not a real MethodTable. GetMethodTableData should fail because
            // the cDAC's type validation rejects this pointer.
