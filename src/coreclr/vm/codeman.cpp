...
namespace {
    struct NibbleMapBase
    {
        protected:
        static DWORD inline Pos2ShiftCount(size_t pos)
        {
            return HIGHEST_NIBBLE_BIT - ((pos & NIBBLES_PER_DWORD_MASK) << LOG2_NIBBLE_SIZE);
        }
    };

    struct LinearLookupNibbleMap : NibbleMapBase
    {
    ...
    struct ConstantLookupNibbleMap : NibbleMapBase
    {
        private:

        static inline size_t GetDwordIndex(size_t relativePointer)
        {
            return relativePointer >> LOG2_BYTES_PER_DWORD;
        }

        static inline size_t GetNibbleIndex(size_t relativePointer)
        {
            return (relativePointer >> (LOG2_BYTES_PER_BUCKET)) & NIBBLES_PER_DWORD_MASK;
        }

        static inline size_t NibbleToRelativeAddress(size_t dwordIndex, size_t nibbleIndex, DWORD nibbleValue)
        {
            return (dwordIndex << LOG2_BYTES_PER_DWORD) + (nibbleIndex << LOG2_BYTES_PER_BUCKET) + ((nibbleValue - 1) << LOG2_CODE_ALIGN);
        }

        static inline DWORD GetNibble(DWORD dword, size_t nibbleIndex)
        {
            return (dword >> POS2SHIFTCOUNT(nibbleIndex)) & NIBBLE_MASK;
        }

        static inline bool IsPointer(DWORD dword)
        {
            return GetNibble(dword, NIBBLES_PER_DWORD - 1) > 8;
        }

        static inline DWORD EncodePointer(size_t relativePointer)
        {
            return (DWORD) ((relativePointer & ~NIBBLE_MASK) + (((relativePointer & NIBBLE_MASK) >> 2) + 9));
        }

        static inline size_t DecodePointer(DWORD dword)
        {
...
TADDR EEJitManager::FindMethodCode(RangeSection * pRangeSection, PCODE currentPC)
{
    ...
    return NibbleMap::FindMethodCode(pHp->pHdrMap, pHp->mapBase, pHp->startAddress, pHp->endAddress, currentPC);
}
...
void EEJitManager::NibbleMapSetUnlocked(HeapList * pHp, TADDR pCode, size_t codeSize)
{
    ...
    NibbleMap::SetUnlocked(pHp->pHdrMap, pHp->mapBase, pCode, codeSize);
}
...
void EEJitManager::NibbleMapDeleteUnlocked(HeapList* pHp, TADDR pCode)
{
    ...
    NibbleMap::DeleteUnlocked(pHp->pHdrMap, pHp->mapBase, pCode);
}
